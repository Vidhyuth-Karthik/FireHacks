/* ============================================================
   ParticleText — vanilla port of the React Bits component.

   Same algorithm, same prop names, same defaults, minus React:
   the useEffect body becomes mountParticleText(container, options)
   and the cleanup function becomes the returned .destroy().

   Kept vanilla so the client stays build-step-free (see UI-SPEC.md).

     import { mountParticleText } from './particle-text.js';
     const pt = mountParticleText(el, { text: 'WHISPER', trigger: 'hover' });
     pt.destroy();
   ============================================================ */
...
/* clamp()/vw font sizes can't be read straight off a string — measure a
   throwaway span so the offscreen canvas gets a real pixel size. */
...
/* Sampling before the webfont lands gives you a fallback-shaped word. */
...
export function mountParticleText(container, options = {}) {
  ...
  srLabel.className = 'particle-text__sr';
  srLabel.textContent = text;

  container.append(canvas, srLabel);
  ...
  /* trigger: 'hover' — pointer entry blows the word apart and re-forms it. */
  ...
  return {
    /** Replay the scatter-and-reform by hand. */
    burst() {
      startGather(true);
    },
    destroy() {
