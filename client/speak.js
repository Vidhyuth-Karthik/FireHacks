...
/* ---- Speech ---------------------------------------------- */
/* Audio is generated by Kokoro on our own backend (see tts.js). The
   browser's speechSynthesis is deliberately unused: this machine only
   offers three robotic SAPI5 voices to it and gender is not selectable. */
...
async function loadVoices() {
  ...
  if (els.voiceSelect) {
    ...
    // Group by "American female", "British male", ... so gender is an
    // explicit choice rather than a guess from a first name.
    const groups = new Map();
    for (const voice of speech.config.voices) {
      const key = voice.group || `${voice.accent ?? ''} ${voice.gender ?? ''}`.trim() || 'Voices';
  ...
  if (!speech.config.online) {
    ...
    trace(`voice list unavailable — ${speech.config.detail || 'using built-in list'}`, 'reject');
  }
}

/* Autoplay policy blocks programmatic play() until the user interacts.
   "Bless" the element once on the first gesture so later playback -
   driven by the joystick, not a click - is allowed. */
...
async function speak(text) {
  ...
  try {
    await tts.speak({
      ...
      speed: 1.0,
      autoplay: true,
      maxChars: speech.config?.maxChars ?? 1000,
    });
    setPill('voice', 'live');
  } catch (error) {
